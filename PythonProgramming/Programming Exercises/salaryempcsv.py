...
while True:
    ...
    # Put your code here
        ...
        try:
            count = len(salary_employees)
        except NameError:
            print("0 employee(s) added to salary_employee.csv")
            openfile.close()
            continue
        for employee in salary_employees:
            attrs = [employee.id, employee.name, employee.monthly_salary]
            openfile.write(my_join(attrs, ",") + "\n")
        openfile.close()
        print(len(salary_employees), " employee(s) added to salary_employee.csv")

    elif selection == 3:
        readfile = open("salary_employee.csv", "r")
        salary_employees = []
        for line in readfile:
            ...
            emp = SalaryEmployee(mylist[0], mylist[1], mylist[2])
